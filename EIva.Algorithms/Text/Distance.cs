...
namespace EIva.Algorithms.Text
{
    /// <summary>
            ...
            where T : IEquatable<T>
        {
            ...
            for (var i = 1; i <= secondLength; i++)
            {
                ...
                for (var j = from; j <= to; j++)
                {
                    ...
                    var cost = firstCh.Equals(secondCh) ? 0 : 1;
                    var value = Math.Min(Math.Min(currentRow[j - 1] + 1, previousRow[j] + 1), previousRow[j - 1] + cost);
